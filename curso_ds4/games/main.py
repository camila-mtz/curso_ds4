...
def main(archivo_torneo:str):
    ...
    if archivo_torneo != "":
        ...
        players_espanya = ['Casillas','Ramos','Pique','Alba','Iniesta','Silva','Isco','Busquets','Costa','Morata','Asensio']
        lista_mexico = [Athlete(x) for x in players_mexico]
        lista_espanya = [Athlete(x) for x in players_espanya]
        soccer = Sport("Soccer", 11, "FIFA")
        mexico = Team("Mexico", soccer, lista_mexico)
        espanya = Team("España", soccer, lista_espanya)
        juego = Game(mexico, espanya)
